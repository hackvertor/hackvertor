...
import hackvertor.ui.ExtensionPanel;
import hackvertor.ui.HackvertorInput;
...
public class HackvertorExtension implements BurpExtension, HttpHandler, IBurpExtender, ITab, IContextMenuFactory, IHttpListener, IExtensionStateListener, IMessageEditorTabFactory {
   ...
   public static final HashMap<String, HashMap<String, Integer>> contextTagCount = new HashMap() {
        ...
        SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                ...
                try {
	            	...
	            	stdout.println("Hackvertor v2.0.0");
                    loadCustomTags();
                    loadGlobalVariables();
                    registerPayloadProcessors();
                    extensionPanel = new ExtensionPanel(hackvertor);

                    callbacks.addSuiteTab(HackvertorExtension.this);
    ...
    public RequestToBeSentAction handleHttpRequestToBeSent(HttpRequestToBeSent requestToBeSent) {
        ...
        print("Tool:" + requestToBeSent.toolSource().toolType().toolName());
        switch (requestToBeSent.toolSource().toolType().toolName()) {
            case "Proxy":
                if (!tagsInProxy) {
        ...
        if (request.indexOf("<@") > -1) {
            print("Found request:" + request);
            hackvertor.setMontoyaRequest(Hackvertor.removeHackvertorTags(request), requestToBeSent);
            request = hackvertor.convert(request, hackvertor);
            HttpRequest modifiedRequest = HttpRequest.httpRequest(requestToBeSent.httpService(), request);
            if (autoUpdateContentLength && modifiedRequest.headers().stream().anyMatch(o -> o.name().equals("Content-Length"))) {
                modifiedRequest.withUpdatedHeader("Content-Length", Integer.toString(modifiedRequest.body().length()));
            }
            RequestToBeSentAction.continueWith(modifiedRequest);
        }
        return RequestToBeSentAction.continueWith(requestToBeSent);
    }
